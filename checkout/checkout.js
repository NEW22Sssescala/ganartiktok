...
async function createCheckoutSession() {
  ...
  if (!COOUD_CONFIG.accessToken || COOUD_CONFIG.accessToken.includes('COLOQUE')) {
    showError('Configura el access token en el archivo config.js');
    return;
  }

  if (!COOUD_CONFIG.prices.length || COOUD_CONFIG.prices[0].includes('COLOQUE')) {
    showError('Configura los IDs de las ofertas en el archivo config.js');
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const hint = response.status === 500 
        ? ' Contacta con el soporte de Cooud para verificar el ID del precio y los permisos del token.'
        : '';
    ...
    if (data.url) {
      ...
      throw new Error('La API no devolvió la URL del checkout.');
    }
  } catch (error) {
    console.error('[Cooud] Erro:', error);
    let msg = error.message || 'Error al crear checkout. Inténtalo de nuevo.';
    if (error.message && error.message.includes('Failed to fetch')) {
      msg = 'No se pudo contactar con el servidor. Si estás usando el proxy, verifica que esté en ejecución: cd checkout && node server-proxy.js';
    }
    showError(msg);
    btn.disabled = false;
...
function getPaymentMethodsLabel() {
  ...
  if (isAndroid) {
    return 'Tarjeta de crédito/débito y Google Pay';
  }
  if (isIOS) {
    return 'Tarjeta de crédito/débito y Apple Pay';
  }
  return 'Tarjeta de crédito/débito, Google Pay y Apple Pay';
}
